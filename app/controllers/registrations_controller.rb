class RegistrationsController < Philia::RegistrationsController
    ...
    def create
      ...
      if !::Philia.use_recaptcha  ||  verify_recaptcha
          ...
          if @tenant.errors.empty?   # tenant created
            if @tenant.plan == 'premium'
              @payment = Payment.new({ email: user_params["email"],
                token: params[:payment]["token"] }) #,tenant: @tenant})
    ...
    def sign_up_params_tenant
      params.require(:tenant).permit(::Philia.whitelist_tenant_params)
    end
    ...
    def sign_up_params_coupon
      (if ::Philia.use_coupon
         params.require(:coupon).permit(::Philia.whitelist_coupon_params)
       else
         params
       end
      )
    ...
    def log_action(action, resource = nil)
      ...
      unless logger.nil?
        logger.debug(
          "PHILIA >>>>> [register user/org] #{action} - #{err_msg}"
        )
      end
    end
